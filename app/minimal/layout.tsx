...
import '../../components/minimal/globals.css';

// Import montserrat font for friendly theme (keeping it consistent with design)
import { Nunito_Sans as nunitoFont } from 'next/font/google';
    ...
    <header>
      <div className='mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8'>
        <Navbar
          ...
          links={[
            { label: 'Features', path: '/minimal/#benefits' },
            { label: 'How it works', path: '/minimal/#solution' },
            { label: 'FAQ', path: '/minimal/#faq' },
...
export default function Layout({ children }: { children: React.ReactNode }) {
  useEffect(() => {
    ...
    document.documentElement.classList.add(nunito.className);
    document.documentElement.classList.add('theme-minimal');

    return () => {
      // Clean up when leaving friendly theme
      document.documentElement.classList.remove(nunito.className);
